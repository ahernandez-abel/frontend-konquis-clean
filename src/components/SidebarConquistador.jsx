...
export const SidebarConquistador = ({ setModuloActivo, moduloActivo, drawerOpen, setDrawerOpen }) => {
  const botones = [
    { name: "perfil", label: "Perfil", icon: <FaUserAlt /> },
  ...
  return (
    <>
      <div className={`sidebar ${drawerOpen ? "open" : ""}`}>
        ...
        {botones.map(btn => (
          <button key={btn.name} className={`sidebar-btn ${moduloActivo === btn.name ? "active" : ""}`} onClick={() => { setModuloActivo(btn.name); setDrawerOpen(false); }}>
